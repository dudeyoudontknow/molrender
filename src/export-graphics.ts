 ...
 * @param path path to file
 */
async function openCif(path: string) {
 ...
 * @param path path to file
 */
async function readCifFile(path: string) {
...
/**
 * OBJGenerator class used to initialize 3dcanvas for rendering
 */
export class OBJGenerator {
    ...
    async renderAsm(modIndex: number, asmIndex: number, models: readonly Model[], outPath: string, id: string) {
        return new Promise<void>(async resolve => {
            ...
            fs.open('objtest.obj', 'w', function(err, fd) {
                if (err) throw err;

                let buffer = new Buffer('#carbohydrates')

                const objList = carbRepr.renderObjects
                for (let i = 0; i < objList.length; i++) {
                    const currObj = objList[i]
                    if (currObj.type === 'mesh') {
                        buffer = new Buffer('mesh_obj 1')

                        fs.write(fd, buffer, buffer.length, null, function(err) {
                            if (err) throw err;
                        })
                        ...
                        for (let i = 2; i < vertices.length; i += 3) {
                            ...
                            buffer = new Buffer(`v ${vertices[i - 2]} ${vertices[i - 1]} ${vertices[i]}`)
                        ...
                        for (let i = 2; i < norms.length; i += 3) {
                            ...
                            buffer = new Buffer(`vn ${norms[i - 2]} ${norms[i - 1]} ${norms[i]}`)
                        ...
                        for (let i = 2; i < faces.length; i += 3) {
                            ...
                            buffer = new Buffer(`f ${faces[i - 2]} ${faces[i - 1]} ${faces[i]}`)
            ...
            fs.open('objtest.obj', 'a', function(err, fd) {
                if (err) throw err;

                let buffer = new Buffer('#main')

                const objList = repr.renderObjects
                for (let i = 0; i < objList.length; i++) {
                    const currObj = objList[i]
                    if (currObj.type === 'mesh') {
                        buffer = new Buffer('mesh_obj 2')
            ...
            fs.open('objtest.obj', 'a', function(err, fd) {
                ...
                let buffer = new Buffer('#ligands')

                const objList = ligandRepr.renderObjects
                for (let i = 0; i < objList.length; i++) {
                    const currObj = objList[i]
                    if (currObj.type === 'mesh') {
                        buffer = new Buffer('mesh_obj 3')
